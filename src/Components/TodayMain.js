...
function TodayMain() {
    ...
    // 투두데이터 중 오늘 날짜의 투두만 보이도록 필터링한 변수  && 배열 뒤집음
    const todayTodoData = todoData.filter((value) => value.createdAt === dateFormat).slice().reverse();
        ...
        <main>
            ...
            <div className="todoCount">
                {currentTab === 0 ?
                    <div className="todoNum">
                        {todayTodoData.length} 개
                    </div>
                    : (currentTab === 1 ?
                        <div className="todoNum">
                            {todayTodoData.filter((value) => checkedItems.includes(value.id)).length} 개
                        </div>
                        : <div className="todoNum">
                            {todayTodoData.filter((value) => !checkedItems.includes(value.id)).length} 개
                        </div>)}
            ...
            {currentTab === 0 ?
                <ul className="todoList">
                    {todayTodoData.slice(offset, offset + limit).map((value) =>
                        <TodoList
                            list={value}
                            key={value.id}
                ...
                : (currentTab === 1 ?
                    <ul className="todoList">
                        {todayTodoData.filter((value) => checkedItems.includes(value.id)).slice(offset, offset + limit).map((value) =>
                            <TodoList
                                list={value}
                                key={value.id}
                                handleCheckChange={handleCheckChange}
